<div id="infrastructure" class="nav-body mb-[3vh] hidden w-full px-[10vw] py-[2vh]">
    <div class="pb-[4vh] flex justify-between">
        <div class="title">
            <h1 class="text-2xl font-bold">Cost of Infrastructure</h1>
        </div>
        <div class="flex gap-x-[1vw]">
            <button class="bg-primary text-white font-semibold px-[1vw] py-[1vh] rounded-md shadow-md hover:bg-blue-600">
                <a
                    href="{{ route('addNewParameter', ['versionId' => $version->id, 'calculationTypeId' => $infrastructure->first()->calculation_type_id]) }}">Add
                    New Parameter</a>
            </button>

            <button class="bg-primary text-white font-semibold px-[1vw] py-[1vh] rounded-md shadow-md hover:bg-blue-600">
                <a
                    href="{{ route('editFormulaView', ['version' => $version->id, 'calculationTypeId' => $infrastructure->first()->calculation_type_id]) }}">Edit
                    Formula</a>
            </button>
        </div>
    </div>

    <form id="infrastructure-form" action="{{ route('detail.update', $version) }}" method="POST">
        @csrf
        <input type="hidden" name="calculation-type" id="calculation-type"
            value={{ $infrastructure->first()->calculation_type_id }}>

        @foreach ($infrastructure as $p)
            @if ($p->parameterType->parameter_type_name == 'Multiple')
                <div class="multiple">
                    ...
                    <button
                        class="bg-primary text-white font-semibold px-[1vw] py-[1vh] rounded-md shadow-md hover:bg-blue-600 mb-5z">
                        <a href="{{ route('addNewSubParameter', ['parameterId' => $p->id, 'calculationTypeId' => $infrastructure->first()->calculation_type_id, 'versionId' => $version->id]) }}"
        ...
        <div class="pt-[5vh] py-[3vh] w-full flex justify-between gap-x-[1vw]">
            <div class="border shadow-md px-[1vw] py-[2vh] border-black w-full rounded-md prompt">
                <p class="font-bold text-lg">Cost of Infrastructure per Course</p>
            </div>
            <div class="w-full">
                <input type="text" name="infrastructure-result" id="infrastructure-result"
                    value="{{ $infrastructureRes ? 'Rp. ' . $infrastructureRes->result : 'Rp. 0' }}"
        ...
        <div class="next flex justify-end pb-[3vh]">
            <div class="flex items-center gap-x-[2vw]">
                ...
                <button type="button" id="save-infrastructure" data-tab="enrollment"
