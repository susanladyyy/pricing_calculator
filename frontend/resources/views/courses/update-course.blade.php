...
@section('content')
    <div class="nav-body w-full px-[10vw] py-[2vh] min-h-[80vh]">
        <h1 class="mb-10 mt-10 text-4xl font-bold">Update Course</h1>

        <form method="POST" action="{{ route('postUpdateCourse', ['id' => $version->id]) }}">
            @csrf
            @error('courseName')
            ...
            <div class="field mb-[3vh] flex gap-x-[1vw]">
                <div class="w-full border border-black shadow-md rounded-md overflow-hidden">
                    <input class="w-full border-x-0 border-t-0" value="{{ $course->course_name }}"
                ...
                <div class="w-full border border-black shadow-md rounded-md overflow-hidden">
                    <input class="w-full border-x-0 border-t-0" value="{{ $course->session_number }}"
                        type="text" name="numberOfSession"
                        id="" placeholder="Example: 12">
                    <p class="py-[1vh] px-[1vw] font-bold">Number of Tutorial Session</p>
                </div>
            </div>

            <div class="w-full border border-black shadow-md rounded-md overflow-hidden">
                <input class="cursor-pointer w-full bg-primary text-white px-[1vw] py-[1vh] rounded-md shadow-md" value="Update Course" type="submit" name="" id="">
            </div>
        </form>
    </div>
@endsection
