...
@section('content')
    <div class="nav-body w-full px-[10vw] py-[2vh] min-h-[80vh]">
        <h1 class="mb-10 mt-10 text-4xl font-bold">Update User</h1>

        <form action="{{ route('postUpdateUser', ['id' => $user->id]) }}" method="POST">
            @csrf
            @error('username')
            ...
            <div class="field mb-[3vh]">
                <div class="border border-black shadow-md rounded-md overflow-hidden">
                    <input class="w-full border-x-0 border-t-0" value="{{ $user->username }}" type="text"
                    ...
                    <select id="select-option" name="role" class="mt-1 block w-full px-4 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-indigo-200 focus:border-indigo-500">
                        ...
                        @foreach ($roles as $r)
                            @if ($user->role_id == $r->id)
                                <option value="{{ $r->id }}" selected>{{ ucfirst($r->role_name) }}</option>
                            @else
            ...
            <div class="field mb-[3vh]">
                <div class="border border-black shadow-md rounded-md overflow-hidden">
                    <input class="w-full border-x-0 border-t-0" value="{{ $user->email }}" type="text"
            ...
            <div class="field mb-[3vh]">
                <div class="border border-black shadow-md rounded-md overflow-hidden">
                    <input class="w-full border-x-0 border-t-0" type="password"
                        id="" name="password" placeholder="Password"/>
                    <p class="py-[1vh] px-[1vw] font-bold">Input your new password (Optional)</p>
                </div>
            </div>
                ...
                <div class="shadow-md rounded-md overflow-hidden">
                    <select id="select-option" name="university" class="mt-1 block w-full px-4 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-indigo-200 focus:border-indigo-500">
                        ...
                        @foreach ($universities as $uni)
                            @if ($user->university_id == $uni->id)
                                <option value="{{ $uni->id }}" selected>{{ $uni->university_name }}</option>
                            @else
            ...
            <div class="w-full border border-black shadow-md rounded-md overflow-hidden">
                <input class="cursor-pointer w-full bg-primary text-white px-[1vw] py-[1vh] rounded-md shadow-md" value="Update User" type="submit">
            </div>
        </form>
    </div>
@endsection
